<template>
    ...
    <AppLayout title="Daftar Dosen Pembimbing" :menu-items="menuItems">
        <DataTable :columns="cols" :data="dosen" searchable :search-keys="['name','email']" class="animate-fade-in-up">
            <template #cell-supervised_students_count="{value}"><span class="badge badge-info">{{ value }} mahasiswa</span></template>
        </DataTable>
    </AppLayout>
</template>
<script setup>
...
defineProps({ dosen: Array });
const cols = [{key:'name',label:'Nama'},{key:'email',label:'Email'},{key:'nomor_induk',label:'NIP'},{key:'supervised_students_count',label:'Bimbingan'}];
const menuItems = [
